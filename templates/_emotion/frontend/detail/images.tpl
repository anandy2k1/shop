...
{if $sArticle.images}
	...
	<div class="thumb_box">
        {if $sArticle.image.src.4}
            ...
            style="background-repeat: no-repeat; background-position: center center; background-color:#fff; background-image: url({$sArticle.image.src.2});"
		...
		{foreach from=$sArticle.images item=sArticleImage}
			{if $sArticleImage.relations}
			    ...
			    <a id="thumb{$sArticleImage.relations}" 
			       ...
			       {if {config name=sUSEZOOMPLUS}}class="cloud-zoom-gallery"{/if}
			       style="background-repeat: no-repeat; background-position: center center; background-color:#fff; background-image: url({$sArticleImage.src.2});">
			    </a>
		    {else}
