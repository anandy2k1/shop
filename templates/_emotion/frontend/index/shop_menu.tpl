        ...
        <div class="topbar_currency">
            {foreach from=$currencies item=currency}
                <form action="" method="post" class="currency">
                    ...
                    <input type="submit" {if $currency->getId() === $shop->getCurrency()->getId()}class="active-2"{/if} class="active-2" value="" value="{$currency->getCurrency()}" />
                </form>
            {/foreach}
        </div> 
    {/if}
