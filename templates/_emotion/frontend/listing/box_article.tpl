...
<p class="{if $sArticle.pseudoprice}pseudoprice{else}price{/if}{if !$sArticle.pseudoprice} both{/if}">
    ...
    {/if}
    <span class="price-from">{if $sArticle.priceStartingFrom && !$sArticle.liveshoppingData}{s name='ListingBoxArticleStartsAt'}{/s} {/if}</span><span class="price">{$sArticle.price|currency} </span><span class="price-star">{s name="Star"}*{/s}</span>
</p>
{/block}
